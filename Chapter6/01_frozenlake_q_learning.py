...
if __name__ == "__main__":
    ...
    while True:
        ...
        if test_reward > 0.95:
            ...
            test_env_wrapped = gym.wrappers.RecordVideo(
            test_env, video_folder="video", 
                name_prefix="final_policy"
            )
            # Record a few episodes with final policy
            for i in range(3):
                agent.play_episode(test_env_wrapped)
            test_env_wrapped.close()
            break
    writer.close()